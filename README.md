...
- **rollback** - roll back a previous deployment using a `$SHA` variable. Creates a revert commit; `sfdx-git-delta` generates the correct delta automatically.
...
`package_check.py` resolves specified tests from source annotations.
