...
| **Static analysis**       | Salesforce Code Analyzer (`code-analyzer.yml`) running the existing PMD ruleset (`scripts/pmd/default_apex.xml`) against Apex plus the Flow engine against Flows, and a SonarQube config (`sonar-project.properties`)                                                                                                                                        |
...
5. **Pick a runner image**. The default is `$CI_REGISTRY_IMAGE:production`, built and pushed to your own (private) registry by the GitLab `build` job from the included `Dockerfile` — use this if you need a private image or want to control the update cadence yourself. Alternatively, pull the prebuilt [`mcarvin8/sf-cli`](https://hub.docker.com/r/mcarvin8/sf-cli) image from Docker Hub (`latest`/`linux` for the Linux variant, `windows`/`windows-ltsc2022` for Windows Server Core) - it's published automatically from this repo's `Dockerfile`/`Dockerfile.windows` and ships with `sf` and the [plugins](#salesforce-cli-plugins) already installed. You can also swap in any other image that has `sf` and the plugins installed.
   - `.github/workflows/` (`docker-publish.yml`, `sync-docker-readme.yml`) publish that `mcarvin8/sf-cli` image and its Docker Hub description - they're specific to my own image and push using my Docker Hub credentials. Safe to delete if you're building your own image via the GitLab `build` job or using another registry; only keep/adapt them if you intend to publish your own equivalent image to Docker Hub.
...
- **code-analyzer-check** - Salesforce Code Analyzer (PMD + Flow engines) static analysis on changed Apex classes/triggers and Flows. Runs on MR pipelines targeting any org branch. Java (PMD) and Python (Flow) plus the `code-analyzer`/`sf-cat` sf CLI plugins are installed at runtime (not baked into the image); all non-PMD/Flow engines (ESLint, CPD, RetireJS, SFGE, ApexGuru) are disabled in `code-analyzer.yml` since Code Analyzer initializes every engine at startup regardless of `--rule-selector` and would otherwise hard-fail on their missing runtimes/auth. Apex rules are the existing `scripts/pmd/default_apex.xml` ruleset, wired in via `code-analyzer.yml` and selected with `--rule-selector pmd:EnforcedApexRules,flow:Recommended` (Code Analyzer tags every rule in a custom ruleset with its `<ruleset name>`, spaces stripped). The `sf-cat` plugin converts the Code Analyzer JSON output into SonarQube's generic issue import format (`sonar-issues.json`), consumed by the `quality` job via `sonar.externalIssuesReportPaths`. The Apex ruleset includes the `UnusedMethod` rule, which requires PMD to know which external namespaces call into your code — otherwise it false-positives on methods invoked by managed packages. The `plugins.dependencies` array in `sfdx-project.json` provides that namespace list. If your project has no managed package dependencies or you remove `UnusedMethod` from the ruleset, clear `plugins.dependencies` to an empty array. `sf code-analyzer run` errors hard on a `--target` directory that doesn't exist or has no scannable files (unlike the old raw `pmd` binary), so `force-app/main/default/triggers/PlaceholderTrigger.trigger` and `force-app/main/default/flows/Placeholder.flow-meta.xml` exist to keep the job working out of the box — delete them once your project has real triggers/Flows.
